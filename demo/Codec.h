...
inline std::string GenKey(const TableId tableId, const HandleId handle, const UInt64 version) {
  auto key = GenKey(tableId, handle);
  {
    auto big_endian_ts = EncodeUInt64Desc(version);
    key.append(reinterpret_cast<const char *>(&big_endian_ts), sizeof(big_endian_ts));
  }
  return key;
}
...
inline std::string DecodeTikvKey(const std::string_view & key) {
...
inline TableId GetTableIdRaw(const std::string & key) {
