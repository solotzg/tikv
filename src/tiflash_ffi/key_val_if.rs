...
use engine::{Peekable, CF_DEFAULT, CF_LOCK, CF_RAFT, CF_TIFLASH, CF_WRITE, DB};
use std::sync::Arc;
...
pub extern "C" fn ffi_handle_put_tiflash_cf(
    ...
    unsafe {
        ...
        match (*proxy_ptr)
            .storage
            .put_cf(handle, key.as_ref(), value.as_ref())
        {
...
pub struct TiFlashRaftProxyHelper {
    proxy_ptr: TiFlashRaftProxyPtr,
    handle_put_tiflash_cf: extern "C" fn(TiFlashRaftProxyPtr, BaseBuffView, BaseBuffView) -> i32,
